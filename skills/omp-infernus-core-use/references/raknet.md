...
```typescript
BitStream.packAspectRatio(v);
BitStream.unpackAspectRatio(v);
BitStream.packCameraZoom(v);
BitStream.unpackCameraZoom(v);
BitStream.packHealthArmour(hp, arm);
BitStream.unpackHealthArmour(val); // → { health, armour }
```
...
onOutgoingPacket(({ playerId, packetId, bs, next }) => {
  return next();
});
onOutgoingRPC(({ playerId, rpcId, bs, next }) => {
  return next();
});
