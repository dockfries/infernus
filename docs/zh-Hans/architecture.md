...
`packages/*` 下 25 个可构建子包，加上 `types`、`shared`、运行时宿主与构建管线。依赖主脊自左向右：基于 `@infernus/core` 的 Gamemode，经 `samp-node` 插件，接入 open.mp 服务端。四个带边框的区域按用途归类各封装子包，每个节点都标注了具体实现——封装了哪个原生，或用了什么机制。
