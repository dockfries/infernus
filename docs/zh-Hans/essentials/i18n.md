...
俄文则是同一机制的另一面：安装俄文字体（russifier）后，[`@infernus/rustext`](../ecosystem) 会改写出站的 `GameText`、`TextDraw` 与 `Menu` RPC 文本，把 cp1251 字节映射成该字体提供的字形。此时必须显式传入 `charset: "cp1251"`——上面的默认字符集会在包拿到文本之前就把西里尔字母变成 `?`。广播给所有玩家的文本（`forAll`）不带玩家 id，不会被转换。
