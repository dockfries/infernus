...
Para `GameText`/`TextDraw`, `Infernus` **no** aplica la conversión de `charset` del jugador: estas dos funciones codifican el texto con el `charset` que les pases (`win1252` por defecto en `GameText` y `ISO-8859-1` en `TextDraw`). El cliente elige los glifos de la textura de su fuente según el valor del byte (similar a los sprites CSS en el desarrollo frontend) en lugar de decodificar los bytes como texto. Algunos mods de cliente (por ejemplo, parches de idioma chino) reemplazan la fuente y esperan bytes UTF-8; en ese caso pasa `charset: "utf8"`.

El ruso funciona con el mismo mecanismo desde el otro lado: un russifier es un mod de localización rusa que el jugador instala en su **cliente**; redibuja la fuente del juego, colocando las letras cirílicas en las celdas que la fuente original reserva para caracteres latinos acentuados. [`@infernus/rustext`](../ecosystem) aporta la mitad del servidor, reescribiendo el texto saliente de las RPC de `GameText`, `TextDraw` y `Menu` para que los bytes cp1251 caigan en las celdas de ese mod. Pasa `charset: "cp1251"` de forma explícita (los valores por defecto anteriores convierten el cirílico en `?` antes de que el paquete pueda verlo) y ajusta el `RussifierType` al paquete que el jugador tenga realmente instalado, porque cada paquete coloca las letras de forma distinta (`SanLtd` es el valor por defecto, junto con `OneC`, `Rush`, `MG` y otros). El paquete no incluye ningún recurso de russifier. El texto enviado a todos los jugadores (`GameText.forAll`) no lleva id de jugador y no se convierte.
