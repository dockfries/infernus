...
El ruso funciona con el mismo mecanismo desde el otro lado: con una fuente russifier instalada, [`@infernus/rustext`](../ecosystem) reescribe el texto saliente de las RPC de `GameText`, `TextDraw` y `Menu`, mapeando los bytes cp1251 a los glifos que ofrece esa fuente. Pasa `charset: "cp1251"` de forma explícita: los valores por defecto anteriores convierten el cirílico en `?` antes de que el paquete pueda verlo. El texto enviado a todos los jugadores (`forAll`) no lleva id de jugador y no se convierte.
