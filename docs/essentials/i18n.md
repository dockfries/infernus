...
Russian works with the same mechanism from the other side: with a russifier font installed, [`@infernus/rustext`](../ecosystem) rewrites the outgoing `GameText`, `TextDraw` and `Menu` RPC text, mapping cp1251 bytes onto the glyphs that font provides. Pass `charset: "cp1251"` explicitly there — the defaults above turn Cyrillic into `?` before the package can convert anything. Text sent to all players (`forAll`) carries no player id and is not converted.
