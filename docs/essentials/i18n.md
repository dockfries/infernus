...
For `GameText`/`TextDraw`, `Infernus` does **not** apply the per-player `charset` conversion. These two APIs encode the text with the `charset` you pass to them — `win1252` by default for `GameText`, `ISO-8859-1` for `TextDraw` — and the client picks glyphs from its font texture by byte value (similar to CSS sprites in frontend development) instead of decoding the bytes as text. Some client mods (e.g., Chinese language patches) replace the font and expect UTF-8 bytes, in which case pass `charset: "utf8"`.

Russian works the same way from the other side. A russifier is a Russian localization mod the player installs on their own **client**; it redraws the game's font, putting Cyrillic letters into the slots the original font reserves for accented Latin characters. [`@infernus/rustext`](../ecosystem) supplies the server half, rewriting the outgoing `GameText`, `TextDraw` and `Menu` RPC text so that cp1251 bytes land on that mod's glyph slots. Pass `charset: "cp1251"` explicitly — the defaults above turn Cyrillic into `?` before the package can convert anything — and set the `RussifierType` that matches the pack the player actually installed, since the packs lay the letters out differently (`SanLtd` is the default, alongside `OneC`, `Rush`, `MG` and others). The package ships no russifier assets. Text sent to all players (`GameText.forAll`) carries no player id and is not converted.
