...
export const IsDynamicActorStreamedIn = (actorId: number, forPlayerId: number): boolean => {
  return !!samp.callNative("IsDynamicActorStreamedIn", "ii", actorId, forPlayerId);
};
...
export const SetDynamicActorVirtualWorld = (actorId: number, vWorld: number): number => {
...
export const SetDynamicActorFacingAngle = (actorId: number, ang: number): number => {
...
export const SetDynamicActorPos = (actorId: number, x: number, y: number, z: number): number => {
  return samp.callNative("SetDynamicActorPos", "ifff", actorId, x, y, z);
};

export const GetDynamicActorHealth = (actorId: number) => {
  const [health, ret]: [number, number] = samp.callNative("GetDynamicActorHealth", "iF", actorId);
  return { health, ret };
};

export const SetDynamicActorHealth = (actorId: number, health: number): number => {
  return samp.callNative("SetDynamicActorHealth", "if", actorId, health);
};

export const SetDynamicActorInvulnerable = (actorId: number, invulnerable = true): number => {
  return samp.callNative("SetDynamicActorInvulnerable", "ii", actorId, invulnerable);
};
...
export const GetDynamicActorAnimation = (actorId: number) => {
  const [
    ...
    time = 0,
    ret,
  ]: [string, string, number, number, number, number, number, number, number] = samp.callNative(
