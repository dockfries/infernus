...
function elevator_OpenDoors() {
  ...
  obj_ElevatorDoors[0].move(x, constants.Y_DOOR_L_OPENED, z, constants.DOORS_SPEED);
  obj_ElevatorDoors[1].move(x, constants.Y_DOOR_R_OPENED, z, constants.DOORS_SPEED);
  return true;
}
...
function elevator_CloseDoors() {
  ...
  const { x, z } = obj_ElevatorDoors[0].getPos();
  obj_ElevatorDoors[0].move(x, constants.Y_DOOR_CLOSED, z, constants.DOORS_SPEED);
  obj_ElevatorDoors[1].move(x, constants.Y_DOOR_CLOSED, z, constants.DOORS_SPEED);
  return true;
}
...
function getElevatorZCoordForFloor(floorId: number) {
  ...
  return constants.GROUND_Z_COORD + constants.FloorZOffsets[floorId] + constants.ELEVATOR_OFFSET;
}
...
export const LSApartments1: ILSApartments1FS = {
  ...
  load(options) {
    ...
    if (options && options.enableCommand) {
      const onCommandText = PlayerEvent.onCommandText("lsa", ({ player, next }) => {
        // Set the interior
        player.setInterior(0);

        // Set player position and facing angle
        player.setPos(1131.07 + Math.random() * 3, -1180.72 + Math.random() * 2, 33.32);
