...
function createPlayerBackgroundTextDraw(
  ...
  txtBackground.setBoxColors(0x00000099).setLetterSize(5.0, 5.0).setFont(0).setShadow(0);
  txtBackground
    .setOutline(0)
    .setColor(0x000000ff)
...
export const VSpawner: IFilterScript = {
  ...
  load() {
    ...
    const onPlayerClickGlobal = TextDrawEvent.onPlayerClickGlobal(({ player, next, textDraw }) => {
    ...
    const vSpawner = PlayerEvent.onCommandText("vspawner", ({ player, next }) => {
