...
const gCurrentPageTextDrawId = new Map<Player, TextDraw>();
const gHeaderTextDrawId = new Map<Player, TextDraw>();
const gBackgroundTextDrawId = new Map<Player, TextDraw>();
const gNextButtonTextDrawId = new Map<Player, TextDraw>();
const gPrevButtonTextDrawId = new Map<Player, TextDraw>();
...
function showPlayerModelPreviews(player: Player) {
  ...
  const gSelectionItemsArr = [];
  const gSelectionItemsTagArr = [];
...
function updatePageTextDraw(player: Player) {
  ...
  const textDraw = gCurrentPageTextDrawId.get(player);
  textDraw?.setString(pageText);
}

function createSelectionMenu(player: Player) {
  gBackgroundTextDrawId.set(
    player,
...
function destroySelectionMenu(player: Player) {
  ...
  gHeaderTextDrawId.get(player)?.destroy();
  gBackgroundTextDrawId.get(player)?.destroy();
  gCurrentPageTextDrawId.get(player)?.destroy();
  gNextButtonTextDrawId.get(player)?.destroy();
  gPrevButtonTextDrawId.get(player)?.destroy();

  gHeaderTextDrawId.delete(player);
  gBackgroundTextDrawId.delete(player);
  gCurrentPageTextDrawId.delete(player);
  gNextButtonTextDrawId.delete(player);
  gPrevButtonTextDrawId.delete(player);
}
...
export const SkinChanger: IFilterScript = {
  ...
  load() {
    ...
    const onPlayerClickPlayer = TextDrawEvent.onPlayerClickPlayer(
      ({ player, textDraw, next }) => {
        ...
        // Handle: next button
        if (textDraw === gNextButtonTextDrawId.get(player)) {
        ...
        if (textDraw === gPrevButtonTextDrawId.get(player)) {
          if (curPage > 0) {
            skinChangePage.set(player, curPage - 1);
            showPlayerModelPreviews(player);
            updatePageTextDraw(player);
  ...
  unload() {
    [...gCurrentPageTextDrawId.values()].forEach((t) => t.destroy());
    [...gHeaderTextDrawId.values()].forEach((t) => t.destroy());
    [...gBackgroundTextDrawId.values()].forEach((t) => t.destroy());
    [...gNextButtonTextDrawId.values()].forEach((t) => t.destroy());
    [...gPrevButtonTextDrawId.values()].forEach((t) => t.destroy());
    [...gSelectionItems.values()].flat().forEach((t) => t.destroy());

    gCurrentPageTextDrawId.clear();
    gHeaderTextDrawId.clear();
    gBackgroundTextDrawId.clear();
    gNextButtonTextDrawId.clear();
    gPrevButtonTextDrawId.clear();
