...
export function registerMiscCommands(options?: IFsDebugOptions) {
  ...
  const goto = PlayerEvent.onCommandText(["goto", "warpto"], ({ player, subcommand, next }) => {
    if (!subcommand[0]) {
      player.sendClientMessage(COLOR_RED, "[USAGE]: /goto PLAYERID (X_OFFSET Y_OFFSET Z_OFFSET)");
      return next();
    }
    ...
    if (!subcommand[1]) {
      ...
      player.sendClientMessage(COLOR_GREEN, `[SUCCESS]: You have warped to ${pName} (ID: ${id}).`);
      return next();
    }
    x += +subcommand[1] || 0;
    if (subcommand[2]) {
  ...
  const setLocation = PlayerEvent.onCommandText("setloc", ({ player, subcommand, next }) => {
    if (!subcommand[0]) {
  ...
  const bring = PlayerEvent.onCommandText("bring", ({ player, subcommand, next }) => {
    if (!subcommand[0]) {
      player.sendClientMessage(COLOR_RED, "[USAGE]: /bring PLAYERID (X_OFFSET Y_OFFSET Z_OFFSET)");
      return next();
    }
    const id = +subcommand[0] || 0;
    if (!Player.isConnected(id)) {
  ...
  const weapon = PlayerEvent.onCommandText(["weapon", "w2"], ({ player, subcommand, next }) => {
    if (!subcommand[0]) {
      player.sendClientMessage(
        COLOR_RED,
