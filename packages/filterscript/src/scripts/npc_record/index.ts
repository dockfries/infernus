...
export const NpcRecord: IFilterScript = {
  name: "npc_record",
  load() {
    const vr = PlayerEvent.onCommandText("vrecord", ({ player, subcommand, next }) => {
    ...
    const ofr = PlayerEvent.onCommandText("ofrecord", ({ player, subcommand, next }) => {
      if (!player.isAdmin()) return false;
      ...
      if (player.isInAnyVehicle()) {
        player.sendClientMessage(0xff0000ff, "Recording: Leave the vehicle and reuse the command.");
        return next();
      }
