...
export const Base: IFilterScript = {
  ...
  load() {
    // PM Command
    const pm = PlayerEvent.onCommandText("pm", ({ player, subcommand, next }) => {
      if (!subcommand.length || subcommand[0].length > 4) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "Usage: /pm (id) (message)");
        return next();
      }
      ...
      if (!Player.isConnected(+id)) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "/pm : Bad player ID");
        return next();
      }

      if (player.id === +id) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "You cannot PM yourself");
        return next();
      }
    ...
    //Kick Command
    const kick = PlayerEvent.onCommandText("kick", ({ player, subcommand, next }) => {
      if (!player.isAdmin()) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "/kick : You are not an admin");
        return next();
      }

      if (!subcommand.length || subcommand[0].length > 4) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "Usage: /kick (id) [reason]");
        return next();
      }
      ...
      if (!Player.isConnected(+id)) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "/kick : Bad player ID");
        return next();
      }
    ...
    //Ban Command
    const ban = PlayerEvent.onCommandText("ban", ({ player, subcommand, next }) => {
      if (!player.isAdmin()) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "/ban : You are not an admin");
        return next();
      }

      if (!subcommand.length || subcommand[0].length > 4) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "Usage: /ban (id) [reason]");
        return next();
      }
      ...
      if (!Player.isConnected(+id)) {
        player.sendClientMessage(ColorEnum.ADMINFS_MESSAGE, "/ban : Bad player ID");
        return next();
      }
