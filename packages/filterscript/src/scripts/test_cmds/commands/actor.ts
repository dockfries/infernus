...
export function createActorCommands() {
  const actoratme = PlayerEvent.onCommandText("actoratme", ({ player, next }) => {
  ...
  const actorground = PlayerEvent.onCommandText("actorground", ({ next }) => {
    test_actor_id?.applyAnimation("BEACH", "ParkSit_M_loop", 4.0, true, true, true, true);
    return next();
  });
  ...
  const actorface = PlayerEvent.onCommandText("actorface", ({ player, next }) => {
    const ang = player.getFacingAngle().angle;
