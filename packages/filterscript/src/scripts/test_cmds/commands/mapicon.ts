import { DynamicMapIcon, PlayerEvent, DynamicCheckpoint, MapIconStyles } from "@infernus/core";

const mapIconArr: DynamicMapIcon[] = [];

export function createMapIconCommands() {
  const mapicontest = PlayerEvent.onCommandText("mapicontest", ({ player, next }) => {
  ...
  const mapicontest2 = PlayerEvent.onCommandText("mapicontest2", ({ player, next }) => {
    const pos = player.getPos();
    if (!pos.ret) return next();
    const { x, y, z } = pos;
