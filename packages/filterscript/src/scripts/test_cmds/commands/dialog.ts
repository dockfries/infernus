...
export function createDialogCommands() {
  ...
  const testmsgbox2 = PlayerEvent.onCommandText("testmsgbox2", async ({ player, next }) => {
    const { response } = await new Dialog({
      style: DialogStylesEnum.MSGBOX,
      caption: "Welcome",
      info: "Welcome:\tInfo\nTest:\t\tTabulated\nLine:\t\tHello",
  ...
  const testinputbox = PlayerEvent.onCommandText("testinputbox", async ({ player, next }) => {
  ...
  const testpassbox = PlayerEvent.onCommandText("testpassbox", async ({ player, next }) => {
    const loginMsg = `Welcome to the SA-MP 0.3 server.\n\n{EEEE88}Account:\t{FFFFFF}${player.getName()}\n\n{FFFFFF}Please enter your password below:`;
    const { response, inputText } = await new Dialog({
  ...
  const testlistbox = PlayerEvent.onCommandText("testlistbox", async ({ player, next }) => {
    const listItems =
  ...
  const testtablist = PlayerEvent.onCommandText("testtablist", async ({ player, next }) => {
  ...
  const testtablistheaders = PlayerEvent.onCommandText(
    "testtablistheaders",
    async ({ player, next }) => {
      const { response, listItem, inputText } = await tabListHeadersDialogTest(player);
  ...
  const testtablistcrash = PlayerEvent.onCommandText(
    "testtablistcrash",
    async ({ player, next }) => {
      const AMMUNATION_SMGS_DIALOG = "Weapon\tAmount\tPrice\n{FFFFFF}MP5\t90\t{FF0000}$3500";
  ...
  const testclosebox = PlayerEvent.onCommandText("testclosebox", ({ player, next }) => {
    Dialog.close(player);
    return next();
  });
