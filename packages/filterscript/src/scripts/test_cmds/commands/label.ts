...
export function createLabelCommands() {
  const labelonvehicle = PlayerEvent.onCommandText("labelonvehicle", ({ player, next }) => {
  ...
  const manylabels = PlayerEvent.onCommandText("manylabels", ({ player, next }) => {
  ...
  const dellabel = PlayerEvent.onCommandText("dellabel", ({ subcommand, next }) => {
  ...
  const playerlabel = PlayerEvent.onCommandText("playerlabel", ({ player, next }) => {
    const pos = player.getPos();
    if (!pos.ret) return next();
    ...
    player.sendClientMessage(0xffffffff, "I created a player label at your position.");
    return next();
  });

  const playerlabelveh = PlayerEvent.onCommandText("playerlabelveh", ({ player, next }) => {
    const veh = player.getVehicle();
    if (!veh) return next();
  ...
  const playerlabelpl = PlayerEvent.onCommandText("playerlabelpl", ({ subcommand, next }) => {
  ...
  const delplayerlabel = PlayerEvent.onCommandText("delplayerlabel", ({ subcommand, next }) => {
    const [txtId] = subcommand;
    const txt3d = Dynamic3DTextLabel.getInstance(+txtId);
    if (txt3d) txt3d.destroy();
    return next();
