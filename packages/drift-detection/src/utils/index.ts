...
import { MIN_DRIFT_ANGLE, MIN_DRIFT_SPEED, DRIFT_TIMEOUT_INTERVAL } from "../constants";
...
export const internalVar = {
  g_DriftPlayers: new SafetyMap<Player, IDriftPlayer>(() => {
    return {
      ...
      lastTimestamp: 0,
      timeoutTicks: 0,
      playerFlags: DriftOptionsEnum.DRIFT_CHECK_ENABLED | DriftOptionsEnum.DAMAGE_CHECK_ENABLED,
    };
  }),

  g_DriftFlags: DriftOptionsEnum.DRIFT_CHECK_ENABLED | DriftOptionsEnum.DAMAGE_CHECK_ENABLED,
...
export function isModelACar(modelId: number) {
  if ([430, 446, 452, 453, 454, 472, 473, 484, 493, 595].includes(modelId)) return false;
  if ([448, 461, 462, 463, 468, 471, 481, 509, 510, 521, 522, 523, 581, 586].includes(modelId))
    return false;
  if (
    [
