...
PlayerEvent.onDialogResponse(({ player, dialogId, listItem, inputText, next }) => {
  if (ACInfo.get(player.id).acKicked > 0) return true;
  const ac_i = Date.now();
  if (ACInfo.get(player.id).acACAllow[49]) {
    if (ac_i - ACInfo.get(player.id).acCall[0] < ac_Mtfc[0][0]) ac_FloodDetect(player, 0);
    else if (ac_i - ACInfo.get(player.id).acCall[27] < ac_Mtfc[27][0]) ac_FloodDetect(player, 27);
    else ACInfo.get(player.id).acFloodCount[0] = ACInfo.get(player.id).acFloodCount[27] = 0;
  }
  if (
