...
PlayerEvent.onClickMap(({ player, next }) => {
  ...
  if (ACInfo.get(player.id).acACAllow[49]) {
    if (ac_gtc - ACInfo.get(player.id).acCall[2] < ac_Mtfc[2][0]) ac_FloodDetect(player, 2);
    else if (ac_gtc - ACInfo.get(player.id).acCall[27] < ac_Mtfc[27][0]) ac_FloodDetect(player, 27);
    else ACInfo.get(player.id).acFloodCount[2] = ACInfo.get(player.id).acFloodCount[27] = 0;
  }
...
PlayerEvent.onClickPlayer(({ player, next }) => {
  ...
  if (ACInfo.get(player.id).acACAllow[49]) {
    ...
    else ACInfo.get(player.id).acFloodCount[3] = ACInfo.get(player.id).acFloodCount[27] = 0;
  }
