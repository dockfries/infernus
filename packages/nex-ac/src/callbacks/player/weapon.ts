...
PlayerEvent.onWeaponShot(({ player, weapon, hitType, hitId, fX, fY, fZ, next }) => {
  if (ACInfo.get(player.id).acKicked > 0 || ACInfo.get(player.id).acDead) return false;
  if (ACInfo.get(player.id).acACAllow[22] && !innerGameModeConfig.ac_LagCompMode) {
    ac_KickWithCode(player, "", 0, 22);
    return false;
  }
  if (
    ACInfo.get(player.id).acACAllow[47] &&
    (!(hitType >= BulletHitTypesEnum.NONE && hitType <= BulletHitTypesEnum.PLAYER_OBJECT) ||
      (hitType === BulletHitTypesEnum.PLAYER && !(hitId >= 0 && hitId < LimitsEnum.MAX_PLAYERS)) ||
      (hitType === BulletHitTypesEnum.VEHICLE &&
        !(hitId >= 1 && hitId < LimitsEnum.MAX_VEHICLES)) ||
      (hitType === BulletHitTypesEnum.OBJECT && !(hitId >= 1 && hitId < LimitsEnum.MAX_OBJECTS)) ||
  ...
  if (ACInfo.get(player.id).acACAllow[34]) {
    ...
    if (
      ...
      ((ac_dist >= 15.0 || ac_zDiff >= 5.0) &&
        ...
        (!surfingPlayerObject || surfingPlayerObject.id === InvalidEnum.OBJECT_ID))
    ) {
      if (++ACInfo.get(player.id).acCheatCount[5] > innerACConfig.AC_MAX_AFK_GHOST_WARNINGS) {
        ...
        triggerCheatWarning(player, "", 0, 34, 0, ACInfo.get(player.id).acCheatCount[5]);
      }
    }
    ...
    if (hitType > BulletHitTypesEnum.NONE) {
      if (
        ...
        if (++ACInfo.get(player.id).acCheatCount[11] > innerACConfig.AC_MAX_SILENT_AIM_WARNINGS) {
          ...
          triggerCheatWarning(player, "", 0, 29, 1, ACInfo.get(player.id).acCheatCount[11]);
        }
      }
    } else {
      if (hitType === BulletHitTypesEnum.PLAYER && hitId !== InvalidEnum.PLAYER_ID) {
        ...
        if (!ACInfo.get(hitId).acDead) {
          if (
            ...
            const ac_dist = hitPlayer.getDistanceFromPoint(ac_hX, ac_hY, ac_hZ);
            ...
            if (
              ...
              (ac_dist >= 20.0 &&
                ...
                (!surfingVehicle || !surfingVehicle.isStreamedIn(hitPlayer)) &&
                (!surfingObject || surfingObject.id === InvalidEnum.OBJECT_ID) &&
                (!surfingPlayerObject || surfingPlayerObject.id === InvalidEnum.OBJECT_ID))
            ) {
              if (++ACInfo.get(player.id).acCheatCount[6] > innerACConfig.AC_MAX_PRO_AIM_WARNINGS) {
                ...
                triggerCheatWarning(player, "", 0, 29, 2, ACInfo.get(player.id).acCheatCount[6]);
              }
  ...
  switch (hitType) {
    case BulletHitTypesEnum.PLAYER: {
      if (
        ...
        const hitPlayer = Player.getInstance(hitId)!;
        if (
          (innerACConfig.AC_USE_NPC && !hitPlayer.isNpc() && !ACInfo.get(hitId).acDead) ||
          !ACInfo.get(hitId).acDead
        ) {
          if (
            ac_gtc - ACInfo.get(hitId).acSetPosTick > ac_gpp &&
            hitPlayer.isInRangeOfPoint(ac_wMinRange[weapon - 22], ac_oX, ac_oY, ac_oZ) &&
            !(
              ACInfo.get(hitId).acSpecAct >= SpecialActionsEnum.ENTER_VEHICLE &&
              ACInfo.get(hitId).acSpecAct <= SpecialActionsEnum.EXIT_VEHICLE
            )
          ) {
            ac_t = player.getTeam();
            if (ac_t === InvalidEnum.NO_TEAM || ac_t !== hitPlayer.getTeam()) {
              if (
          ...
          if (
            ...
            if (
              ...
              const {
                ...
                y: ac_hY,
                z: ac_hZ,
              } = Vehicle.getModelInfo(ac_t, VehicleModelInfoEnum.WHEELSFRONT);
              const {
                x: ac_oX,
                y: ac_oY,
                z: ac_oZ,
              ...
              if (
                ...
                ((ac_wX === 0.0 && ac_wY === 0.0 && ac_wZ === 0.0) ||
                  (ac_GetVectorDist(ac_wX - fX, ac_wY - fY, ac_wZ - fZ) > 1.2 &&
                    ac_GetVectorDist(-ac_wX - fX, ac_wY - fY, ac_wZ - fZ) > 1.2))
              )
                ac_ur2 = true;
            }
  ...
  if (ACInfo.get(player.id).acACAllow[47]) {
    if (ac_t === PlayerStateEnum.DRIVER) {
      ...
      if (
        (ACInfo.get(player.id).acLastWeapon !== weapon && ac_t !== PlayerStateEnum.PASSENGER) ||
        ...
        if (++ACInfo.get(player.id).acCheatCount[16] > innerACConfig.AC_MAX_FAKE_WEAPON_WARNINGS) {
          ...
          triggerCheatWarning(player, "", 0, 47, 2, ACInfo.get(player.id).acCheatCount[16]);
        }
        return false;
    ...
    if (ACInfo.get(player.id).acShotWeapon === weapon) {
      if (weapon !== WeaponEnum.MINIGUN && ac_t !== PlayerStateEnum.PASSENGER) {
        if (ac_gtc - ACInfo.get(player.id).acReloadTick < 110) {
          ...
          if (++ACInfo.get(player.id).acCheatCount[8] > innerACConfig.AC_MAX_RAPID_FIRE_WARNINGS) {
            ...
            triggerCheatWarning(player, "", 0, 26, 1, ACInfo.get(player.id).acCheatCount[8]);
          }
        }
      ...
      if (
        ...
        if (++ACInfo.get(player.id).acCheatCount[12] > innerACConfig.AC_MAX_AUTO_C_WARNINGS) {
          ...
          triggerCheatWarning(player, "", 0, 26, 2, ACInfo.get(player.id).acCheatCount[12]);
        }
      }
    }
  } else if (ac_i < 30) {
    ...
    if (ACInfo.get(player.id).acAmmo[ac_s] === 0) {
      ...
      if (weapon === WeaponEnum.MINIGUN) {
        if (++ACInfo.get(player.id).acCheatCount[7] > 9) {
          ...
          triggerCheatWarning(player, "", 0, 17, 2, ACInfo.get(player.id).acCheatCount[7]);
        }
      }
  ...
  if (ACInfo.get(player.id).acAmmo[ac_s] !== 0) ACInfo.get(player.id).acAmmo[ac_s]--;
  if (ACInfo.get(player.id).acAmmo[ac_s] < -32768) ACInfo.get(player.id).acAmmo[ac_s] += 65536;
