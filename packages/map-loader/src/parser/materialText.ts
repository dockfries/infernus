...
export function materialTextParser(obj: DynamicObject, line: string[], options: IMapLoadOptions) {
  ensureLength("materialTextParser", line, 10, line.length);

  let [materialIndex, materialSize, fontFace, fontSize, bold, fontColor, backColor, textAlignment] =
    line.slice(1);
