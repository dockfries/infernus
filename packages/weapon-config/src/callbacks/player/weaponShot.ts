...
import { s_WeaponRange, s_MaxWeaponShootRate, s_WeaponDamage } from "../../constants";
...
PlayerEvent.onWeaponShot(({ player, weapon, hitType, hitId, fX, fY, fZ, next }) => {
  ...
  if (hitType === BulletHitTypesEnum.PLAYER && hitId !== InvalidEnum.PLAYER_ID) {
    if (!orig_playerMethods.isConnected.call(Player.getInstance(hitId))) {
      addRejectedHit(player, hitId, RejectedReasonEnum.HIT_DISCONNECTED, weapon, hitId);
      return 0;
    }
  ...
  if (hitType < BulletHitTypesEnum.NONE || hitType > BulletHitTypesEnum.PLAYER_OBJECT) {
    addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_INVALID_HITTYPE, weapon, hitType);
  ...
  if (beingReSynced.get(player.id)) {
    addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_BEING_RESYNCED, weapon);
    return 0;
  }

  if (!wc_IsPlayerSpawned(player) && tick - lastDeathTick.get(player.id) > 80) {
    addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_NOT_SPAWNED, weapon);
    return 0;
  }

  if (!isBulletWeapon(weapon)) {
    addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_INVALID_WEAPON, weapon);
  ...
  const length = GameMode.vectorSize(fOriginX - fHitPosX, fOriginY - fHitPosY, fOriginZ - fHitPosZ);
  const origin_dist = GameMode.vectorSize(fOriginX - x, fOriginY - y, fOriginZ - z);
  ...
  if (origin_dist > 15.0) {
    ...
    if ((!in_veh && (!suf_obj || suf_obj.id === InvalidEnum.OBJECT_ID)) || origin_dist > 50.0) {
      addRejectedHit(
        player,
        damagedId,
  ...
  if (hitType !== BulletHitTypesEnum.NONE) {
    ...
    if (hitType === BulletHitTypesEnum.PLAYER) {
      if (
        ...
        addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_SAME_VEHICLE, weapon);
        return 0;
      }
      ...
      const suf_veh = orig_playerMethods.getSurfingVehicle.call(Player.getInstance(hitId)!);
      const in_veh =
        orig_playerMethods.isInAnyVehicle.call(Player.getInstance(hitId)!) ||
        (suf_veh && suf_veh.id !== InvalidEnum.VEHICLE_ID);

      if (dist > 20.0) {
        const suf_obj = orig_playerMethods.getSurfingObject.call(Player.getInstance(hitId)!);
        if ((!in_veh && (!suf_obj || suf_obj.id === InvalidEnum.OBJECT_ID)) || dist > 50.0) {
          addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_TOO_FAR_FROM_SHOT, weapon, dist);
  ...
  let idx = (lastShotIdx.get(player.id) + 1) % lastShotTicks.get(player.id).length;
  ...
  if (hitType === BulletHitTypesEnum.VEHICLE) {
    if (
      ...
      addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_INVALID_VEHICLE, weapon, hitId);
      return 0;
    }

    if (!orig_vehicleMethods.isStreamedIn.call(Vehicle.getInstance(hitId)!, player)) {
      addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_UNSTREAMED, weapon, hitId);
      return 0;
    }
    ...
    if (!vehicle || hitId === vehicle.id) {
      addRejectedHit(player, damagedId, RejectedReasonEnum.HIT_OWN_VEHICLE, weapon);
      return 0;
    }
